    ...
    <main className="min-h-screen bg-background">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
          ...
          {items.map((row) => (
            <li key={row.user.id} className="px-4 py-3 flex items-center gap-3 cursor-pointer hover:bg-muted/30" onClick={() => openUserDetail(row.user.id, row.user.name)}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-muted-foreground mt-1">{row.voteCount} votos • última vez: {formatDate(row.lastVoteAt)}</div>
              </div>
            </li>
          ))}
        </ol>
        ...
        {!hasMore && items.length === 0 && !loading && (
          <div className="text-center text-muted-foreground">No hay datos aún</div>
        )}
